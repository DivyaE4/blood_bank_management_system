...
from forms import LoginForm, RegisterForm, AdminLoginForm, RequestForm  # Assuming you have these forms created
...
def camp_details(id):
    ...
    if request.method == 'POST':
        ...
        # Update the donation count for the camp
        cursor.execute("UPDATE donation_camps SET donation_count = donation_count + 1 WHERE id = %s", (id,))
        conn.commit()
...
def update_inventory():
    ...
    # Update inventory based on aggregated results using a nested query
    for (blood_type, total_units) in results:
        # Nested query to update or insert the quantity for each blood type
        cursor.execute("""
            UPDATE inventory 
            SET qty = (
                SELECT COUNT(*) FROM donations WHERE blood_type = %s
            )
            WHERE blood_type = %s
        ...
        if cursor.rowcount == 0:
            cursor.execute("""
                INSERT INTO inventory (blood_type, qty)
                VALUES (%s, %s)
            """, (blood_type, total_units))
